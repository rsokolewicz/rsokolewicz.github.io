...
$$\begin{multline}
    \hat{H}
        = \sum_{\\{i,j\\}\in\text{n.n.}}\,\Big(\frac{J_{\text{ex}}}{n}\hat{\bb{S}}_i\cdot\hat{\bb{S}}_j - \frac{K}{2n}\left(\hat{{S}}_{z,i}\right)^2%\cdot\hat{\bb{S}}_{z,j}%-t_h \hat{c}_i^\dagger\hat{c}_j
...
$$\begin{equation}
	\hbar \partial_t \bb{m} = \bb{m}\times\big(-\gamma\\,\bb{H}_\text{ext}+KS \\,\bb{m_z} + \Delta\_{\text{sd}}\\,\bb{s}\big)
\end{equation}$$
...
$$\begin{equation}
	G^\text{R(A)}_{\bb{p}} = \frac{1}{\varepsilon-H\_{\bb{p}}-\Sigma^\text{R(A)}}.
\end{equation}$$
...
By choosing a basis that satisfies the normalization condition $\tr\bb{B}\_i\bb{B}\_j=n\delta\_{ij}$ we find that the response matrix $\mathcal{F}$ has the following form
